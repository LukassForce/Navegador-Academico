...
app.put('/editarJustificacion/', bodyParser.json(), (req, res) => {
    let id = req.body.id;
    ...
    connection.query("UPDATE justificacion SET nombre=?,apellido=?,curso=?,asunto=?,mensaje=? WHERE id =?", data, (error, res1) => {
        if (error)
            throw error;
        if (!res1) {
...
app.post('/crearUsuario', bodyParser.json(), (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    connection.query("INSERT INTO usuario (rut, password, nombre, apellido, curso) VALUES('" + rut + "','" + passwordHash + "','" + nombre + "','" + apellido + "','" + curso + "')", (req1, res1) => {
...
app.get('/usuario/:rut', (req, res) => {
    ...
    connection.query("SELECT * FROM usuario WHERE rut = ?", rut, (error, res1) => {
        if (error)
            throw error;
        if (!res1) {
